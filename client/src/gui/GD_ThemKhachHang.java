...
public class GD_ThemKhachHang extends JFrame implements ActionListener, MouseListener {
	...
	private KhachHang_DAO_Interface khachhang_dao;
	private DefaultTableModel modelSach;
	private JButton btnThem;
	private JComboBox cbo_loai;
	private LoaiKhachHang_DAO_Interface loaikhachhang_dao;
	...
	public static void ThemKHScreen() {
		...
		public GD_ThemKhachHang() {
			...
			setResizable(false);
			khachhang_dao= lookupKhachHangDAO();
			loaikhachhang_dao= lookupLoaiKhachHangDAO();
			...
			panel_1.add(btnHienTatCa);
			 ...
			 txt_email.setColumns(10);
				List<LoaiKhachHang> listKH;
				try {
					listKH = loaikhachhang_dao.getallLoaiKhachHang();
			...
			try {
				...
				for(KhachHang kh:list)
				{
					modelKhachHang.addRow(new Object[] {
							kh.getMaKH(), kh.getTenKH(), kh.getSDT(), kh.getEmail(), kh.getLoaiKhachHang().getTenLoaiKH(),kh.isGioiTinh() ? "Nam":"Nữ" , kh.getDiaChi()
							});
				}
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
		...
		private void themKH()
		{
			...
			maLoaiKhachHang="";
			LoaiKhachHang_DAO_Interface dsKH = loaikhachhang_dao;
			List<LoaiKhachHang> listLoaiKH;
			try {
				listLoaiKH = dsKH.getallLoaiKhachHang();
		...
		private void suaKH()
		{
			...
			String tenLoaiKH=cbo_loai.getSelectedItem().toString();
			maLoaiKhachHang="";
			LoaiKhachHang_DAO_Interface dsKH = lookupLoaiKhachHangDAO();
			List<LoaiKhachHang> listLoaiKH;
			try {
				listLoaiKH = dsKH.getallLoaiKhachHang();
				for (LoaiKhachHang loaiKH : listLoaiKH) {
		...
		private void timKhachHangTheoMa() {
			...
			List<KhachHang> list;
			try {
				list = (List<KhachHang>) khachhang_dao.getKhachHangTheoMa(txtTimKiem.getText());
		...
		private void timKhachHangTheoTen() {
			...
			List<KhachHang> list;
			try {
				list = khachhang_dao.getKhachHangTheoTen(txtTimKiem.getText());
				if (txtTimKiem.getText().equals("")) 
		...
		public void mouseClicked(MouseEvent e) {
			...
			/*try {
				cbo_loai.setSelectedItem(loaikhachhang_dao.layTenLoaiKhachHangTuMaKhachHang(modelKhachHang.getValueAt(row, 4).toString()));
		...
		private boolean validatekhachHang() {
		 ...
		 private LoaiKhachHang_DAO_Interface lookupLoaiKhachHangDAO() {
		        try {
		            Registry registry = LocateRegistry.getRegistry("localhost");
		            return (LoaiKhachHang_DAO_Interface) registry.lookup("LoaiKhachHangDAO");
		        } catch (Exception e) {
		            e.printStackTrace();
		            return null;
		        }
