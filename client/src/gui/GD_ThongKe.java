...
public class GD_ThongKe extends JFrame  implements ActionListener{
	...
	public GD_ThongKe() {
        ...
        String thang = String.valueOf(currentMonth);
        String nam = String.valueOf(currentYear);
		...
		txtThang.setSelectedItem(thang);
		...
		try {
			...
			HoaDon_DAO_Interface hoaDonDAO = lookupHoaDonDAO();
    		tongTienCuaThangHienTai(thang , nam);


            List<Object[]> results = hoaDonDAO.getHoaDonByMonthAndYear(thang, nam);


            DefaultCategoryDataset dataset = new DefaultCategoryDataset();
            for (Object[] result : results) {
                dataset.addValue((Number) result[1], "Doanh thu", result[0].toString());
            }

            JFreeChart chart = ChartFactory.createBarChart("Biểu đồ doanh thu", "Tháng", "Tổng tiền", dataset, PlotOrientation.VERTICAL, false, true, true);
		...
		// Gọi các phương thức để hiển thị thông tin thống kê
		thongKeThangNay();
	...
	private void hienThiBieuDo() {
        try {
        	 String thang = txtThang.getSelectedItem().toString();
 	        String nam = txtNam.getSelectedItem().toString();
 	        HoaDon_DAO_Interface hoaDonDAO = lookupHoaDonDAO();
            List<Object[]> results = hoaDonDAO.getHoaDonByMonthAndYear(thang, nam);
	...
	private void tongTienCuaThang() {
	    HoaDon_DAO_Interface hoadon_dao = lookupHoaDonDAO();
	    String month = txtThang.getSelectedItem().toString();
	    String year = txtNam.getSelectedItem().toString();
	    List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongTienCacHoaDon(month, year);
			for (HoaDon HD : listHD) {
		        DecimalFormat de = new DecimalFormat("###,### VNĐ");
		        Double stringdouble = Double.valueOf(HD.getSum());
		        lblTongTien1.setText(de.format(stringdouble));
		    }
	...
	private void tongTienCuaThangHienTai(String thang, String nam) {
	    HoaDon_DAO_Interface hoadon_dao = lookupHoaDonDAO();
	    List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongTienCacHoaDon(thang, nam);
			for (HoaDon HD : listHD) {
		        DecimalFormat de = new DecimalFormat("###,### VNĐ");
		        Double stringdouble = Double.valueOf(HD.getSum());
		        lblTongTien1.setText(de.format(stringdouble));
	...
	private void tongHoaDonNgayHomQua() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongSoHoaDonNgayHomQua();
			for(HoaDon HD: listHD)
			{
	...
	private void tongHoaDonNgayHomNay() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongSoHoaDonNgayHomNay();
			for(HoaDon HD: listHD)
			{
				String stringInt= Integer.toString(HD.getSoHoaDon());	
	...
	private void tongHoaDonNgayTuanNay() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongSoHoaDonTuanNay();
			for(HoaDon HD: listHD)
			{
				String stringInt= Integer.toString(HD.getSoHoaDon());	
	...
	private void tongHoaDonNgayThangNay() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongSoHoaDonThangNay();
			for(HoaDon HD: listHD)
			{
				String stringInt= Integer.toString(HD.getSoHoaDon());	
	...
	private void tongDoanhThuCuaHomNay() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongDoanhThuHomNay();
			for(HoaDon HD: listHD)
			{
	...
	private void tongDoanhThuCuaHomQua() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongDoanhThuHomQua();
			for(HoaDon HD: listHD)
			{
				 de = new DecimalFormat("###,### VNĐ");
				 Double stringdouble= Double.valueOf(HD.getSum());	
	...
	private void tongDoanhThuCuaTuanNay() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongDoanhThuTuanNay();
			for(HoaDon HD: listHD)
			{
				 de = new DecimalFormat("###,### VNĐ");
				 Double stringdouble= Double.valueOf(HD.getSum());	
	...
	private void tongDoanhThuCuaThangNay() {
		...
		List<HoaDon> listHD;
		try {
			listHD = hoadon_dao.getTongDoanhThuThangNay();
			for(HoaDon HD: listHD)
			{
				 de = new DecimalFormat("###,### VNĐ");
				 Double stringdouble= Double.valueOf(HD.getSum());	
	...
	private void thongKe() {
		...
		List<HoaDon> list;
		try {
			list = hoadon_dao.getHoaDonTheoThangVaNam(month, year);
			if (txtThang.getSelectedItem().equals("")) 
			 ...
			 {	
				...
				for(HoaDon hd : list){
				  String [] rowData = {hd.getMaHD(), hd.getNguoiLapHD().getTenNV(), hd.getNgayLapHD()+"", hd.isUuDai() ? "Có ưu đãi": "Không có ưu đãi", hd.getTenSach().getTenSach(), hd.getTongTien()+""};
	...
	private void thongKeThangNay() {
		...
		List<HoaDon> list;
		try {
			list = hoadon_dao.getallHoaDonThangNay();
			for(HoaDon hd : list){
				  String [] rowData = {hd.getMaHD(), hd.getNguoiLapHD().getTenNV(), hd.getNgayLapHD()+"", hd.isUuDai() ? "Có ưu đãi": "Không có ưu đãi", hd.getTenSach().getTenSach(), hd.getTongTien()+""};
				  modelThongKe.addRow(rowData);			
				}
	...
	private HoaDon_DAO_Interface lookupHoaDonDAO() {
	    try {
	        Registry registry = LocateRegistry.getRegistry("192.168.43.84", 1099);
