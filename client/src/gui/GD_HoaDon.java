...
public class GD_HoaDon extends JFrame implements ActionListener, MouseListener {
	...
	private HoaDon_DAO_Interface hoadon;
	...
	public void DocDuLieuHoaDonVaoTable() {
		...
		try {
			...
			for (HoaDon hd : list) {
				modelHoaDon.addRow(new Object[] { hd.getMaHD(), hd.getNguoiLapHD().getTenNV(), hd.getNgayLapHD(),
						hd.isUuDai() ? "Có " : "Không", hd.getTenSach().getTenSach(), hd.getKhachHang().getTenKH(),
						hd.getTongTien() });
			}
	...
	private void DocDuLieuHoaDonTuanGanNhatVaoTable() {
		...
		List<HoaDon> list;
		try {
			list = hoadon_dao.getallHoaDonTrongTuanGanNhat();
			for (HoaDon hd : list) {
				modelHoaDon.addRow(new Object[] { hd.getMaHD(), hd.getNguoiLapHD().getTenNV(), hd.getNgayLapHD(),
						hd.isUuDai() ? "Có " : "Không", hd.getTenSach().getTenSach(), hd.getKhachHang().getTenKH(),
						hd.getTongTien() });
	...
	private void DocDuLieuHoaDonNgayHomNayVaoTable() {
		...
		List<HoaDon> list;
		try {
			list = hoadon_dao.getallHoaDonNgayHomNay();
			for (HoaDon hd : list) {
	...
	public void DocDuLieuHoaDonDaHuyVaoTable() {
		List<HoaDon> list;
		try {
			list = hoadon_dao.getallHoaDonDaHuy();
			for (HoaDon hd : list) {
	...
	private void tim() {
		HoaDon_DAO_Interface hoaDon = lookupHoaDonDAO();
		List<HoaDon> list;
		try {
			list = hoaDon.getHoaDonTheoMa(txtTimKiem.getText());
	...
	private void timHDDaHuy() {
		...
		List<HoaDon> list;
		try {
			list = hoaDon.getHoaDonDaHuyTheoMa(txtTimKiemHDDaHuy.getText());
